...
void * thread_c(void * arg_in)
{
    ...
    do {
        ...
        while (arg->status != S_GUESS) {
            pthread_cond_wait(&arg->cond_guess, &arg->mutex); //wait
        }
        guess = arg->guess; //get guess from shared mem
        pthread_mutex_unlock(&arg->mutex);
        result = gmn_check(&gmn, guess);  //      call gmn_check()
        ...
        arg->status = S_RESULT;
        pthread_cond_signal(&arg->cond_result);           // wake up Thread P
        pthread_mutex_unlock(&arg->mutex);
...
void * thread_p(void *arg_in)
{
    ...
    do { 
        ...
        // wait for result (MUST be inside the lock)
        while (arg->status != S_RESULT) {
            pthread_cond_wait(&arg->cond_result, &arg->mutex);
        }
...
int main(int argc, char *argv[])
{
    ...
    check_pthread_return(pthread_mutex_init(&arg.mutex, NULL), "mutex init");
    check_pthread_return(pthread_cond_init(&arg.cond_guess, NULL), "cond_guess init");
    check_pthread_return(pthread_cond_init(&arg.cond_result, NULL), "cond_result init");

    check_pthread_return(
    pthread_create(&tid_c, NULL, thread_c, &arg),
    "pthread_create c"
);

    check_pthread_return(
        pthread_create(&tid_p, NULL, thread_p, &arg),
        "pthread_create p"
    );

    // wait for both threads to finish
    check_pthread_return(pthread_join(tid_p, NULL), "join p");
    check_pthread_return(pthread_join(tid_c, NULL), "join c");
