...
void * thread_main(void * arg_in)
{
    ...
    // send max value
    snprintf(buf, sizeof(buf), "%d\n", gmn_get_max());// conv max value in string
    if (send_str(sockfd, buf) < 0) { // send max value(server send porion of buffer up to /0)
        close(sockfd); free(arg); return NULL;
    }
    //  repeat the following until guess is correct
    int result;
    do{
        if (recv_lines(sockfd, rec_buf,sizeof(rec_buf))<0) {
            close(sockfd); free(arg); return NULL;
        }//  wait for a guess

        int guess = atoi(rec_buf);

        result = gmn_check(&gmn, guess);//      call gmn_check()
        snprintf(buf, sizeof(buf), "%d\n", result);// convert result into char ->string in buffer
    ...
    send_str(sockfd, gmn_get_message(&gmn));// final msg
